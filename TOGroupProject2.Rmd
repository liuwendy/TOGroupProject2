...
library(gmodels)
library(caret)
logreg_model <- glm(Attrition_Flag1 ~ . +
...
LOG_prediction <- predict(logreg_model, credit_test, type="response")
...
log_train <- train(Attrition_Flag1 ~ ., data = credit_norm, method = "lm", 
            metric = "Rsquared",
            trControl = log_ctrl,
            preProcess = c("center","scale")
            )
log_train
log_pred <-predict(log_train, credit_norm)
```
...
knn_train <- train(as.factor(Attrition_Flag1) ~ ., data = credit_norm, method = "knn",
           metric = "Kappa",
           trControl = knn_ctrl,
           preProcess = c("center","scale"), 
           tuneLength = 20,
...
## ANN Train
```{r}
ann_ctrl <- trainControl(method = "cv", number = 4)
ann_grid <- expand.grid(.size = c(1, 5, 7, 9, 10, 15, 20),
                        .decay = c(1, 5, 7, 9, 10, 15, 20))

ann_train <- train(as.factor(Attrition_Flag1) ~ ., data = credit_norm, method = "nnet",
          metric = "Kappa",
          trControl = ann_ctrl,
          tuneGrid = ann_grid)
...
ann_pred <- predict(ann_train, credit_norm)
```
...
```{r}
svm_control <- trainControl(method = "cv", number = 4)

svm_cv <- train(as.factor(Attrition_Flag1) ~ ., 
                data = credit_train,
                method = "svmLinear",
...
dt_train <- train(as.factor(Attrition_Flag1) ~ ., data = credit_norm, method = "C5.0",
                  metric = "Kappa",
                  trControl = dt_ctrl,
                  tuneGrid = dt_grid)

dt_train_pred <- predict(dt_train, credit_norm)
table(dt_train_pred, credit_norm$Attrition_Flag1)

dt_train_prob <- head(predict(dt_train, credit_norm, type = "prob"))
...
library(C50)
DT_model2 <- C5.0(as.factor(credit_test.Attrition_Flag1) ~ ., data=credit_train1, costs = error_cost)
plot(DT_model2)
