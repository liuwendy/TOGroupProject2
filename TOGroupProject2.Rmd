...
As we have more than 10000 observation in the original dataset, we decided to first split our data into train and test, in a 50:50 ratio. The primary reason for this step is to avoid over-fitting which will occur if we train and test on the same dataset. To clarify, since both the train and test datasets will still contain a high amount of observations, it will not affect our the accuracy of our later procedures when using trainControl and stack modeling. 
...
confusionMatrix(as.factor(credit_test$Attrition_Flag1), knn_pred)
```
